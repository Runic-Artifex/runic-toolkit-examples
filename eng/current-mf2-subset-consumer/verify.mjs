...
const nugetIdentities = ["RunicTranslations", "RunicTranslations.Build", "RunicTranslations.Generator", "RunicTranslations.Tool"];
const npmIdentity = "@runic-artifex/vite-plugin-runic-translations";

const catalog = JSON.stringify({
  schemaVersion: 2, catalog: "w40", code: { namespace: "W40.Consumer", className: "W40Text", visibility: "public" },
  defaultLocale: "en", locales: [{ tag: "en" }, { tag: "de", fallback: "en" }], layers: [{ name: "base", priority: 0 }],
  validation: { translationCompleteness: "error", extraLocaleKeys: "error", emptyValues: "error" },
}, null, 2) + "\n";

function resource(locale, welcome) {
  return JSON.stringify({
    schemaVersion: 3, catalog: "w40", locale, layer: "base", resources: {
      Greeting: {
        Welcome: { $value: { mf2: { profile, ast: { astVersion: 3, profile, inputs: { name: { type: "string", format: "none" } }, declarations: [], selectors: [], variants: [{ matches: {}, pattern: [{ kind: "text", value: welcome }, { kind: "expression", operand: { kind: "input", name: "name" } }] }] } } } },
      },
    },
  }, null, 2) + "\n";
}

const invalidFullMf2 = resource("en", "Hello ").replace(profile, "full-mf2/1");
const program = `using System.Text.Json;\nusing W40.Consumer;\nusing RunicTranslations;\nvar en = new W40Text(await W40TextCatalog.CreateManagerAsync("en"));\nvar de = new W40Text(await W40TextCatalog.CreateManagerAsync("de"));\nConsole.WriteLine(JsonSerializer.Serialize(new { catalog = W40TextCatalog.CatalogId, contractFingerprint = W40TextCatalog.ContractFingerprint, runtimeAbiVersion = W40TextCatalog.RuntimeAbiVersion, generatorVersion = W40TextCatalog.GeneratorVersion, locales = new[] { "de", "en" }, messages = new { en = new { welcome = en.Greeting.Welcome("Ada") }, de = new { welcome = de.Greeting.Welcome("Ada") } } }));\n`;

const project = version => `<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net10.0</TargetFramework><OutputType>Exe</OutputType><ImplicitUsings>enable</ImplicitUsings><Nullable>enable</Nullable><IntermediateOutputPath>obj/</IntermediateOutputPath><TranslationsEmitEsm>true</TranslationsEmitEsm><TranslationsOutputPath>obj/$(TargetFramework)/translations</TranslationsOutputPath></PropertyGroup><ItemGroup><PackageReference Include="RunicTranslations" Version="${version}"/><PackageReference Include="RunicTranslations.Build" Version="${version}" PrivateAssets="all"/><PackageReference Include="RunicTranslations.Generator" Version="${version}" PrivateAssets="all" OutputItemType="Analyzer" ReferenceOutputAssembly="false"/></ItemGroup><ItemGroup><TranslationCatalog Include="Resources/w40.catalog.json"/><TranslationDocument Include="Resources/w40.en.json"/><TranslationDocument Include="Resources/w40.de.json"/><TranslationsToolInput Include=".config/dotnet-tools.json"/></ItemGroup></Project>\n`;
const toolManifest = version => JSON.stringify({ version: 1, isRoot: true, tools: { "runictranslations.tool": { version, commands: ["runic-translations"] } } }, null, 2) + "\n";
...
export function verifyReceipt(receipt) {
  ...
  for (const journey of journeys) {
    ...
    if (journey.profile !== profile || journey.manifest?.catalog !== "w40" || !/^sha256:[a-f0-9]{64}$/.test(journey.manifest?.contractFingerprint ?? "") || journey.manifest?.esmAbiVersion !== 2 || journey.manifest?.runtimeAbiVersion !== 1 || journey.manifest?.generatorVersion !== 1) errors.push("catalog contract mismatch");
    if (!same(journey.parity?.locales, ["de", "en"]) || !same(journey.parity?.messages, expectedMessages())) errors.push("C# and ESM parity mismatch");
    if (!same(journey.negativeGates, ["unsupported-full-mf2", "abi-mismatch", "schema-mismatch", "stale-generated-manifest", "forged-generated-manifest"])) errors.push("negative gate evidence mismatch");
...
async function writeFixture(directory, version, registry) {
  ...
  for (const [path, content] of [["Resources/w40.catalog.json", catalog], ["Resources/w40.en.json", resource("en", "Hello ")], ["Resources/w40.de.json", resource("de", "Hallo ")], ["Resources/w40.full-mf2.json", invalidFullMf2], [".config/dotnet-tools.json", toolManifest(version)]]) {
...
async function generatedManifest(path, csharp) {
  ...
  if (manifest.webModuleManifestVersion !== 1 || manifest.esmAbiVersion !== 2 || manifest.catalog !== csharp.catalog || manifest.contractFingerprint !== csharp.contractFingerprint || !Array.isArray(manifest.assets)) throw new Error("generated manifest contract failed closed");
...
async function esmParity(directory, manifestPath, csharp) {
  const plugin = await import(pathToFileURL(join(directory, "node_modules", ...npmIdentity.split("/"), "index.js")).href);
  ...
  if (runtime.contractFingerprint !== csharp.contractFingerprint || runtime.esmAbiVersion !== 2 || !same(runtime.locales, csharp.locales)) throw new Error("generated ESM contract mismatch");
  const actual = { locales: runtime.locales, messages: { en: { welcome: messages.m["Greeting.Welcome"]({ name: "Ada" }, { locale: "en" }) }, de: { welcome: messages.m["Greeting.Welcome"]({ name: "Ada" }, { locale: "de" }) } } };
...
async function negatives(directory, manifestPath, environment) {
  ...
  const rejects = async (name, mutate) => { await mutate(); try { const plugin = await import(pathToFileURL(join(directory, "node_modules", ...npmIdentity.split("/"), "index.js")).href); await assertRejects(() => plugin.runicTranslations({ manifest: manifestPath }).buildStart.call({ addWatchFile() {} })); return name; } finally { await writeFile(manifestPath, originalManifest); await writeFile(messagesPath, originalMessages); } };
  const unsupported = await run("dotnet", ["tool", "run", "runic-translations", "--", "validate", "--catalog", "Resources/w40.catalog.json", "--documents", "Resources/w40.full-mf2.json"], directory, environment); if (unsupported.exitCode === 0 || !unsupported.output.includes("RTR0030")) throw new Error("unsupported full MF2 was accepted");
  return ["unsupported-full-mf2", await rejects("abi-mismatch", async () => writeFile(manifestPath, JSON.stringify({ ...manifest, esmAbiVersion: 99 }))), await rejects("schema-mismatch", async () => writeFile(manifestPath, JSON.stringify({ ...manifest, webModuleManifestVersion: 99 }))), await rejects("stale-generated-manifest", async () => writeFile(messagesPath, Buffer.concat([originalMessages, Buffer.from("// stale\n")]))) , await rejects("forged-generated-manifest", async () => writeFile(manifestPath, JSON.stringify({ ...manifest, contractFingerprint: `sha256:${"0".repeat(64)}` })))] ;
}
...
async function nugetProvenance(directory, version) {
  ...
  return Promise.all(nugetIdentities.map(async identity => {
    ...
    if (identity === "RunicTranslations.Tool") return { identity, version, source: "exact-local", contentHash: await sha512(archive), archiveSha256: await sha256(archive) };
...
async function candidateVersion() { const files = await readdir(nugetFeed); const match = files.map(name => /^RunicTranslations\.([0-9][0-9A-Za-z.-]*)\.nupkg$/.exec(name)).find(Boolean); if (!match || !nugetIdentities.every(identity => files.includes(`${identity}.${match[1]}.nupkg`))) throw new Error("RUNIC_W40_NUGET_FEED must contain one exact local RunicTranslations candidate family."); return match[1]; }

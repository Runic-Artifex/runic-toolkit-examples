...
export const BRIDGE_CONTRACT = { identity: 'runic.artifex.setup', version: 1, fingerprint: 'f92970461e801b80f1e8b8fbf7bab346dece692b61c3c4c167b093ce6bc29336' };
const root = resolve(import.meta.dirname, '../..');
const hostTransport = resolve(import.meta.dirname, '../current-host-transport');
const generator = resolve(root, '../runic-toolkit/eng/generate-application-bridge-contract.mjs');
...
const consumer = [
  "import assert from 'node:assert/strict'; import { readFile } from 'node:fs/promises'; import { createEnvironmentInjector, runInInjectionContext } from '@angular/core'; import { Effect, Schema } from 'effect'; import { MockApplicationBridge, TestApplicationBridge, bridgeError, createApplicationBridgeController, defineApplicationContract } from '@runic-artifex/application-bridge'; import { injectApplicationBridge, provideApplicationBridge } from '@runic-artifex/angular';",
  "const manifest=JSON.parse(await readFile(process.argv[2],'utf8')); assert.deepEqual({identity:manifest.protocol.identity,version:manifest.protocol.version,fingerprint:manifest.contractFingerprint},{identity:'runic.artifex.setup',version:1,fingerprint:'f92970461e801b80f1e8b8fbf7bab346dece692b61c3c4c167b093ce6bc29336'}); const Snapshot=Schema.Struct({viewId:Schema.String,revision:Schema.Int,selectedFeatures:Schema.Array(Schema.String),canNavigateBack:Schema.Boolean,canNavigateNext:Schema.Boolean}); const Command=Schema.Union(Schema.TaggedStruct('InitializeApplication',{}),Schema.TaggedStruct('Navigate',{target:Schema.String,expectedRevision:Schema.Int})); const Receipt=Schema.TaggedStruct('NavigationAccepted',{snapshot:Snapshot}); const Event=Schema.TaggedStruct('NavigationChanged',{viewId:Schema.String,revision:Schema.Int}); const contract=defineApplicationContract({identity:manifest.protocol.identity,version:manifest.protocol.version,fingerprint:manifest.contractFingerprint,command:Command,receipt:Receipt,event:Event,snapshot:Snapshot,initialize:{_tag:'InitializeApplication'}}); const initial={viewId:'Welcome',revision:0,selectedFeatures:[],canNavigateBack:false,canNavigateNext:true}; const complete={...initial,viewId:'Complete',revision:1,canNavigateBack:true};",
...
export function verifyReceipt(receipt, authority) {
  ...
  const expected = [['generated-contract', ['node', '<authoritative-generator>', 'current-host-transport/contract.mjs', 'current-host-transport/generated', '--check']], ['npm-install', ['npm', 'install', '--ignore-scripts']], ['angular-controller', ['node', 'consumer.mjs', 'generated/bridge.manifest.json']]];
...
export async function runAngularController(manifestPath) {
  ...
  try {
    const generatedResult = await run('node', [generator, join(hostTransport, 'contract.mjs'), join(hostTransport, 'generated'), '--check'], root); requireSuccess('generated-contract', generatedResult); const bridgeManifest = JSON.parse(await readFile(join(hostTransport, 'generated', 'bridge.manifest.json'), 'utf8')); if (!same({ identity: bridgeManifest.protocol?.identity, version: bridgeManifest.protocol?.version, fingerprint: bridgeManifest.contractFingerprint }, BRIDGE_CONTRACT)) throw new Error('W20-002 generated contract identity mismatch');
    ...
    const phases = []; for (const [name, command, args] of [['npm-install', 'npm', ['install', '--ignore-scripts']], ['angular-controller', 'node', ['consumer.mjs', 'generated/bridge.manifest.json']]]) { const result = await run(command, args, directory, environment); phases.push(phase(name, [command, ...args], result)); requireSuccess(name, result); }
    ...
    const receipt = { schema: RECEIPT_SCHEMA, feeds: { npm: NPM_FEED }, isolation: { npmCache: '.npm-cache' }, releaseManifest: await releaseManifestAfter(manifestPath, authority), bridgeContract: BRIDGE_CONTRACT, bridgeManifestSha256: await sha256(join(hostTransport, 'generated', 'bridge.manifest.json')), candidates, phases: [phase('generated-contract', ['node', '<authoritative-generator>', 'current-host-transport/contract.mjs', 'current-host-transport/generated', '--check'], generatedResult), ...phases] }; const report = verifyReceipt(receipt, receipt.releaseManifest); if (!report.ok) throw new Error(report.errors.join('\n')); return receipt;

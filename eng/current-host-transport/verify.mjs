...
export const MANIFEST = JSON.parse(await readFile(join(import.meta.dirname, 'generated', 'bridge.manifest.json'), 'utf8'));
...
const contractGenerator = resolve(process.env.RUNIC_CURRENT_HOST_TRANSPORT_CONTRACT_GENERATOR ?? join(root, '../runic-toolkit/eng/generate-application-bridge-contract.mjs'));
...
const client = [
  "import assert from 'node:assert/strict'; import net from 'node:net'; import { readFile } from 'node:fs/promises'; import { Schema } from 'effect'; import { ApplicationBridgeLive, createApplicationBridgeController, createWebSocketFrameChannel, defineApplicationContract } from '@runic-artifex/application-bridge';",
  "const [url, manifestPath, fixturesPath] = process.argv.slice(2); const manifest = JSON.parse(await readFile(manifestPath, 'utf8')); const fixtures = await Promise.all(['initialize.client.json','resynchronized.host.json','late-old-admission-error.host.json','future-admission-error.host.json'].map(async (name) => JSON.parse(await readFile(fixturesPath + '/' + name, 'utf8')))); assert.equal(manifest.generatorFormatVersion, 1); assert.ok(manifest.schemas.length >= 20); for (const value of fixtures) { assert.equal(value.protocol, manifest.protocol.identity); assert.equal(value.version, manifest.protocol.version); assert.equal(value.contractFingerprint, manifest.contractFingerprint); } assert.equal(fixtures[2].connectionEpoch, 0); assert.equal(fixtures[3].connectionEpoch, 2);",
  "const snapshot = Schema.Struct({ viewId: Schema.String, revision: Schema.Int, selectedFeatures: Schema.Array(Schema.String), canNavigateBack: Schema.Boolean, canNavigateNext: Schema.Boolean }); const contract = defineApplicationContract({ identity: manifest.protocol.identity, version: manifest.protocol.version, fingerprint: manifest.contractFingerprint, command: Schema.Union(Schema.TaggedStruct('InitializeApplication', {}), Schema.TaggedStruct('Navigate', { target: Schema.String, expectedRevision: Schema.Int })), receipt: Schema.TaggedStruct('NavigationAccepted', { snapshot }), event: Schema.TaggedStruct('NavigationChanged', { viewId: Schema.String, revision: Schema.Int }), snapshot, initialize: { _tag: 'InitializeApplication' } });",
...
export async function runCurrentHostTransport(manifestPath) {
  ...
  try {
    ...
    const phases = [];
    const generated = await run('node', [contractGenerator, join(import.meta.dirname, 'contract.mjs'), join(import.meta.dirname, 'generated'), '--check'], root, environment);
    phases.push(phase('generated-contract', ['node', '<authoritative-generator>', 'contract.mjs', 'generated', '--check'], generated)); requireSuccess('generated contract', generated);
    ...
    const clientResult = await run('node', ['client.mjs', host.url.replace(/^http/, 'ws') + '/bridge', join(directory, 'generated', 'bridge.manifest.json'), join(directory, 'fixtures')], directory, environment); phases.push(phase('transport', ['node', 'client.mjs', '<local-host>', 'generated/bridge.manifest.json', 'fixtures'], clientResult)); requireSuccess('transport', clientResult);

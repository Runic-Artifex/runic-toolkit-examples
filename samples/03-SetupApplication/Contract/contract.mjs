...
const commands = [
  { tag: "InitializeApplication", schema: Schema.TaggedStruct("InitializeApplication", {}), receipt: "ApplicationInitialized", startsOperation: false, cancellable: false, advancesRevision: false },
  { tag: "SelectDestination", schema: Schema.TaggedStruct("SelectDestination", { currentSelectionId: Schema.optional(Uuid) }), receipt: "DestinationSelected", startsOperation: false, cancellable: false, advancesRevision: true },
  { tag: "Navigate", schema: Schema.TaggedStruct("Navigate", { target: SetupViewId, expectedRevision: Revision }), receipt: "NavigationAccepted", startsOperation: false, cancellable: false, advancesRevision: true },
  { tag: "StartInstallation", schema: Schema.TaggedStruct("StartInstallation", { destinationSelectionId: Uuid, selectedFeatures: Schema.Array(FeatureId) }), receipt: "InstallationStarted", startsOperation: true, cancellable: true, advancesRevision: true },
  { tag: "CancelOperation", schema: Schema.TaggedStruct("CancelOperation", { operationId: Uuid }), receipt: "OperationCancellationAccepted", startsOperation: false, cancellable: false, advancesRevision: false },
];
const receipts = [
...
const errors = ["TransportUnavailable", "TransportClosed", "ProtocolVersionMismatch", "ProtocolDecodeError", "CommandRejected", "StaleRevision", "OperationFailed", "OperationCancelled", "OperationTimedOut"]
  .map((tag) => ({ tag, schema: Schema.TaggedStruct(tag, { message: Schema.String, retryable: Schema.Boolean }) }));
